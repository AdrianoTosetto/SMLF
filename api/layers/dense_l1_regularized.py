...
'''
    this is a slightly different use of decorator pattern for applying L1 regularization where the 
    decoratee is created within the decorator __init__ method
'''

class DenseL1Regularized(SMLFComponent, CommonLayer):
    ...
    def regularization_term_weights(self,):
        return self.w_regularizer *  np.sum(np.abs(self.decoratee.weights))

    def regularization_term_biases(self,):
        return self.b_regularizer * np.sum(np.abs(self.decoratee.biases))
