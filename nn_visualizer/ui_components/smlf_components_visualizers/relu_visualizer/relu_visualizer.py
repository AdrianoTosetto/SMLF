...
class ReLUVisualizer(CanvasDrawable):
    def __init__(self,
            units: int,
        ...
        self.layout_data.set_draw_backward_connections_strategy(draw_single_connected_backward_connection)

    def _build_drawable_units(self):
        units_origins = self.layout_data.get_units_origins()

        return [LayerVisualizerUnit(self.layout_data, index, self.on_node_selection_callback) for index in range(len(units_origins))]
...
# class ReLUVisualizer(CanvasDrawable):
#     def __init__(self, units: int, layout_data: BatchNormalizationVisualizerLayoutData, origin = (0, 0), parent = None):
#         CanvasDrawable.__init__(self)
#         self.layout_data = layout_data
#         self.previous_layer_units_origins = []
#         self.x, self.y = layout_data.get_origin()
...
#         self.padding: tuple[float, float, float, float] = (.0, .0, .0, .0)
...
#         return [BatchNormalizationUnitVisualizer(x, y, self.layout_data) for (x, y) in units_origins]
...
#     def get_padding_left(self) -> int:
#         padding_left, _, _, _ = self.padding

#         return padding_left

#     def get_padding_right(self) -> int:
#         _, _, padding_right, _ = self.padding

#         return padding_right

#     def get_padding_top(self) -> int:
#         _, padding_top, _, _ = self.padding

#         return padding_top

#     def get_padding_bottom(self) -> int:
#         _, _, _, padding_bottom = self.padding

#         return padding_bottom

#     def draw(self, painter: QPainter):
#         title = 'Batch Norm'
#         (title_x, title_y) = self.layout_data.get_origin()
#         point = QPoint(int(title_x), int(title_y) - 10)
